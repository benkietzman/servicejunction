      ...
      if (ptJson->m.find("Password") != ptJson->m.end() && !ptJson->m["Password"]->v.empty())
      {
        if (ptJson->m.find("Domain") != ptJson->m.end() && !ptJson->m["Domain"]->v.empty())
        {
          if (ptJson->m.find("Function") != ptJson->m.end() && !ptJson->m["Function"]->v.empty())
          {
            ...
            samba.init(strIp, strShare, ptJson->m["Domain"]->v, ptJson->m["User"]->v, ptJson->m["Password"]->v);
            // {{{ directoryExist
            if (ptJson->m["Function"]->v == "directoryExist")
            {
              if (strPath.empty() || samba.directoryExist(strPath))
              {
                bProcessed = true;
              }
              else
            ...
            {
              list<string> items = samba.directoryList(strPath);
              bProcessed = true;
            ...
            else if (ptJson->m["Function"]->v == "establishDirectory")
            {
              if (!strPath.empty())
              {
                if (samba.establishDirectory(strPath))
                {
                  bProcessed = true;
                }
                else
            ...
            else if (ptJson->m["Function"]->v == "login")
            {
              ...
              else if (samba.fileExist(strFile))
              {
                bProcessed = true;
              }
              else
            ...
            else if (ptJson->m["Function"]->v == "makeDirectory")
            {
              if (!strPath.empty())
              {
                if (samba.makeDirectory(strPath))
                {
                  bProcessed = true;
                }
                else
            ...
            else if (ptJson->m["Function"]->v == "rename")
            {
              if (ptRequest->m.find("OldPath") != ptRequest->m.end() && !ptRequest->m["OldPath"]->v.empty())
              {
                ...
                {
                  if (samba.rename(ptRequest->m["OldPath"]->v, ptRequest->m["NewPath"]->v))
                  {
                    bProcessed = true;
                  }
                  else
